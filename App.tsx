...
const SPEED_PRESETS = [0.75, 0.85, 1.0, 1.15, 1.25];
...
const App: React.FC = () => {
  ...
  const [isSpeedApplied, setIsSpeedApplied] = useState(false);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-indigo-500/30">
        ...
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <nav className="flex items-center bg-slate-800/50 p-1 rounded-xl border border-slate-700">
            <button 
              ...
              className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${activeTab === 'translator' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-slate-200'}`}
            >
              <Languages size={14} /> Dịch Phụ Đề
            </button>
            <button 
              onClick={() => setActiveTab('speed-editor')}
              className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${activeTab === 'speed-editor' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-slate-200'}`}
            >
          ...
          <div className="flex items-center gap-4">
             ...
             {(blocks.length > 0 || analysis) && (
               <button onClick={() => window.location.reload()} className="p-2 rounded-lg text-slate-400 hover:text-red-400 hover:bg-red-400/10 transition-all">
                 <Trash2 size={18} />
               </button>
             )}
          </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <section className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
            {!fileName ? (
              ...
              >
                ...
                <p className="text-xs text-slate-400 font-bold uppercase tracking-widest text-center">Tải file .SRT</p>
            ...
            <div className="space-y-6 animate-in slide-in-from-left-4 duration-500">
              <section className={`bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl transition-all ${!fileName ? 'opacity-30 pointer-events-none' : ''}`}>
                <h2 className="text-xs font-bold mb-6 flex items-center gap-2 text-indigo-400 uppercase tracking-widest">
                  <FileSearch size={16} /> Phân tích Tiêu đề
                </h2>
                {status.isAnalyzing ? (
                  <div className="flex items-center justify-center py-8 gap-3 text-indigo-400">
                    <Loader2 className="animate-spin" size={20} />
                    <span className="text-xs font-bold uppercase tracking-widest">AI đang thẩm thấu nội dung...</span>
                  </div>
                ) : analysis ? (
                  <div className="space-y-4">
                    <div className="p-4 bg-slate-800/40 border border-slate-700 rounded-2xl space-y-4 shadow-inner">
                      <div className="space-y-1">
                        ...
                        <p className="text-sm font-bold text-white pl-2 break-words">{detectedTitle}</p>
                      </div>
                      <div className="space-y-1">
                        <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">🇻🇳 TIÊU ĐỀ VIỆT GỢI Ý:</span>
                        <p className="text-lg font-bold text-indigo-400 pl-2">{analysis.translatedTitle}</p>
                      </div>
                  ...
                  <button onClick={() => handleAnalyze(detectedTitle)} className="w-full py-3 bg-indigo-600/20 text-indigo-400 border border-indigo-600/30 rounded-2xl text-[10px] font-bold uppercase tracking-widest">
                    Bấm để Phân tích Cốt truyện
                  </button>
                )}
              </section>
                ...
                <div className="space-y-4">
                  ...
                  {status.fileStatus === 'completed' && (
                    <div className="px-4 py-2 bg-indigo-500/10 border border-indigo-500/30 rounded-xl flex items-center gap-2 text-indigo-400">
                      ...
                      <span className="text-[9px] font-bold uppercase tracking-widest">✅ FILE ĐÃ HOÀN TẤT</span>
                    </div>
                  )}

                  {!status.isTranslating && status.fileStatus !== 'completed' && (
                    <button onClick={startTranslation} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2 shadow-xl shadow-indigo-500/20">
                      <Play size={20} fill="currentColor" /> {status.fileStatus === 'mixed' ? 'Dịch tiếp phần còn lại' : 'Bắt đầu dịch AI'}
                    </button>
                  )}
                    ...
                    <div className="space-y-3 p-5 bg-slate-800/30 rounded-2xl border border-slate-700/50">
                      <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest text-slate-400">
                        <div className="flex items-center gap-2 text-indigo-400">
                          <Loader2 className="animate-spin" size={14} /> <span>Tiến trình: {status.progress}/{status.total}</span>
                        </div>
                        <span>{progressPercentage}%</span>
                      </div>
                      <div className="h-2 bg-slate-800 rounded-full overflow-hidden shadow-inner">
                        <div className="h-full bg-indigo-600 transition-all duration-300" style={{ width: `${progressPercentage}%` }} />
                      </div>
                    </div>
                  )}
                      ...
                      <div className="space-y-3 text-[11px] text-slate-300 border-t border-red-500/10 pt-3">
                        ...
                        <div className="space-y-1">
                          ...
                          <ul className="pl-4 opacity-80">
                            ...
                            <li>• Đã dịch: {status.interruption.translated}</li>
                            <li>• Còn lại: {status.interruption.remaining}</li>
                          </ul>
                        </div>
                        <div className="p-3 bg-red-500/5 border border-red-500/10 rounded-xl italic opacity-70">
                          Tải file dở dang → chờ 1 phút → upload lại → bấm “Dịch tiếp phần còn lại”.
                        </div>
                      </div>
                      <button onClick={() => downloadTranslated(true)} className="w-full py-3 bg-red-600 hover:bg-red-700 text-white rounded-2xl text-[10px] font-bold uppercase tracking-widest flex items-center justify-center gap-2">
                  ...
                  {status.fileStatus === 'completed' && !status.isTranslating && (
                    <button onClick={() => downloadTranslated(false)} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2 shadow-xl">
          ...
          {activeTab === 'speed-editor' && (
            <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
              <section className={`bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl transition-all ${!fileName ? 'opacity-30 pointer-events-none' : ''}`}>
                <h2 className="text-xs font-bold mb-6 flex items-center gap-2 text-indigo-400 uppercase tracking-widest">
                  <Clock size={16} /> Cấu hình Tốc độ
                </h2>
                <div className="space-y-6">
                  <div className="space-y-3">
                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">⏱️ Nhập tốc độ (Playback Speed):</label>
                    <div className="flex items-center gap-4">
                      <input 
                        type="number" 
                        step="0.01"
                        ...
                        className="flex-1 bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-lg font-bold text-white focus:outline-none focus:border-indigo-500"
                      />
                    ...
                    <div className="flex flex-wrap gap-2">
                      {SPEED_PRESETS.map(val => (
                        <button 
                          ...
                          className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all ${speed === val ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-slate-800 border-slate-700 text-slate-400 hover:text-slate-200'}`}
                        >
                          {val.toFixed(2)}x
                        </button>
                      ))}
                  ...
                  <div className="p-4 bg-slate-800/30 border border-slate-700/50 rounded-2xl flex gap-3 text-slate-400 text-[11px] italic">
                    ...
                    <p>Hệ thống sẽ chia toàn bộ timestamp cho {speed.toFixed(2)}. Ví dụ: 10s sẽ trở thành {(10/speed).toFixed(2)}s.</p>
                  </div>

                  <button 
                    onClick={applySpeed}
                    className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2 shadow-xl shadow-indigo-500/20"
                  >
                    <Zap size={20} fill="currentColor" /> Áp dụng & Tải file mới
                  </button>
                </div>
              </section>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
            {blocks.length === 0 ? (
              ...
              blocks.slice(0, 100).map((block) => {
                ...
                return (
                  <div key={block.index} className={`grid grid-cols-1 md:grid-cols-2 gap-6 p-6 rounded-3xl border transition-all duration-700 ${isTranslated ? 'border-emerald-500/20 bg-emerald-500/5 shadow-inner' : 'border-slate-800 bg-slate-900/40'}`}>
                    <div className="space-y-3">
                    ...
                    <div className="md:border-l border-slate-800 md:pl-8 space-y-3 flex flex-col justify-center min-h-[60px]">
                      <div className="flex justify-between items-center">
                        <span className={`text-[9px] uppercase font-bold tracking-widest ${isTranslated ? 'text-emerald-500' : 'text-slate-600'}`}>
                          {isTranslated ? 'Bản dịch tối ưu' : activeTab === 'speed-editor' ? 'Chưa thay đổi' : 'Đang chờ dịch...'}
                        </span>
            ...
            {blocks.length > 100 && (
              <div className="p-4 text-center text-slate-600 text-[10px] uppercase font-bold tracking-[0.3em]">
                ... Hiển thị giới hạn 100 block xem trước ...
              </div>
            )}
          </div>
        </div>
      ...
      <footer className="p-4 text-center border-t border-slate-900 bg-slate-950/80">
        <p className="text-slate-700 text-[9px] uppercase tracking-[0.5em] font-bold">Donghua AI Toolset • Speed Editor & Translator Engine</p>
      </footer>
    </div>
  );
};
